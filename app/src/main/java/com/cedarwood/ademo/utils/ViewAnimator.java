...
public class ViewAnimator {
     ...
     */
    public void animateViewIfNecessary(final int position, @NonNull final View view, @NonNull final Animator[] animators, int type) {
        if (mShouldAnimate && position > mLastAnimatedPosition) {
            if (mFirstAnimatedPosition == -1) {
                mFirstAnimatedPosition = position;
            }
//            Log.i("GAB", "4 Delay[" + position + "]=" + type+"|");
            animateView(position, view, animators,type);
     ...
     */
    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    private void animateView(final int position, @NonNull final View view, @NonNull final Animator[] animators, int type) {
        ...
        set.setStartDelay(calculateAnimationDelay2(position,type));
    ...
    private int calculateAnimationDelay(final int position) {
        ...
        Log.i(TAG,"firstVisiblePosition : "+firstVisiblePosition+"  lastVisiblePosition : "+lastVisiblePosition
                +"  mFirstAnimatedPosition : "+mFirstAnimatedPosition+"  mLastAnimatedPosition : "+mLastAnimatedPosition+"  position : "+position);
    ...
    @SuppressLint("NewApi")
    private int calculateAnimationDelay2(final int position,int type) {
        int delay = 0;

        int lastVisiblePosition = ((LinearLayoutManager)mRecyclerView.getLayoutManager()).findLastCompletelyVisibleItemPosition();
        int firstVisiblePosition = ((LinearLayoutManager)mRecyclerView.getLayoutManager()).findFirstCompletelyVisibleItemPosition();

        Log.i(TAG,"Delay firstVisiblePosition : "+firstVisiblePosition+"  lastVisiblePosition : "+lastVisiblePosition
                +"  mFirstAnimatedPosition : "+mFirstAnimatedPosition+"  mLastAnimatedPosition : "+mLastAnimatedPosition
                +"  position : "+position);


        if(startPosition == 0 || type==0){
            int delaySinceStart = (position - startPosition) * mAnimationDelayMillis;
            delay = Math.max(0, (int) (-SystemClock.uptimeMillis() + mAnimationStartMillis + mInitialDelayMillis + delaySinceStart));
            ...
            Log.i("GAB", "7 Delay[" + position + "]=" + delay+"|"+delaySinceStart+"|"+startPosition+"|"+type+"|"+mAnimationDelayMillis);
        }else{
            delay = mAnimationDelayMillis;
            if (mRecyclerView.getLayoutManager() instanceof GridLayoutManager) {
                int numColumns = ( (GridLayoutManager)mRecyclerView.getLayoutManager()).getSpanCount();
                int delaySinceStart =  mAnimationDelayMillis * (position % numColumns);
                delay +=  mInitialDelayMillis + delaySinceStart;
                Log.d("GAB", "8 Delay[" + position + "]=*"+delay+"|"+delaySinceStart+"|"+startPosition+"|"+type);
            }

            Log.d("GAB", "6 Delay[" + position + "]=*"+delay+"|"+startPosition+"|"+type);
        }


        Log.i("GAB", "4 Delay[" + position + "]=" + delay+"|"+lastVisiblePosition+"|"+firstVisiblePosition+"|"+mAnimationDelayMillis);
        return delay;
    }
