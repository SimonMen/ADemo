...
import com.cedarwood.ademo.utils.AnimationUtil;
import com.cedarwood.ademo.utils.ViewAnimator;
...
public abstract class AnimatorAdapter<T extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<T> {
    ...
    private void animateView(final View view, final int position) {
        ...
        Animator[] concatAnimators = AnimationUtil.concatAnimators(animators, alphaAnimator);

        int type = mRecyclerView.getLayoutManager() instanceof GridLayoutManager
                || mRecyclerView.getLayoutManager() instanceof StaggeredGridLayoutManager ?1:0;

        mViewAnimator.animateViewIfNecessary(position, view, concatAnimators,type);
    }
